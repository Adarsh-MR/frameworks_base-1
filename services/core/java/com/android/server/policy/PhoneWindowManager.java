...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    private void readConfigurationDependentBehaviors() {
        ...
        final boolean hasHome = mHasNavigationBar || (mDeviceHardwareKeys & KEY_MASK_HOME) != 0;
        final boolean hasAppSwitch = mHasNavigationBar || (mDeviceHardwareKeys & KEY_MASK_APP_SWITCH) != 0;
        final boolean hasBack = mHasNavigationBar || (mDeviceHardwareKeys & KEY_MASK_BACK) != 0;
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
        ...
        if (canApplyCustomPolicy(keyCode) && !isCustomSource) {
            if (mNavBarEnabled /* TODO> && !isADBVirtualKeyOrAnyOtherKeyThatWeNeedToHandleAKAWhenMonkeyTestOrWHATEVER! */) {
                if (DEBUG_INPUT) {
                    Log.d(TAG, "interceptKeyBeforeQueueing(): key policy: mHasNavigationBar, discard hw event.");
                }
